...
CDialogDemoDlg::CDialogDemoDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CDialogDemoDlg::IDD, pParent)
	, m_sName(_T(""))
{
...
BEGIN_MESSAGE_MAP(CDialogDemoDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BTN_TEST, &CDialogDemoDlg::OnBnClickedBtnTest)
END_MESSAGE_MAP()
...
void CDialogDemoDlg::OnBnClickedBtnTest()
{
	/*
	CString str;
	((CEdit *)GetDlgItem(IDC_EDIT_NAME))->GetWindowText(str);
	MessageBox(str);
	*/
	UpdateData(TRUE);		//从控件取值放到成员m_sName中
	MessageBox(m_sName);
	m_sName += "|尾巴";
	UpdateData(FALSE);		//将成员m_sName的值赋给控件
}
