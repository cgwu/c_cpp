...
SdiSquares.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 CSquaresApp 应用程序类。

SdiSquares.cpp
    这是包含应用程序类 CSquaresApp 的主应用程序源文件。
...
SquaresDoc.h、SquaresDoc.cpp - 文档
    这些文件包含 CSquaresDoc 类。编辑这些文件以添加特殊文档数据并实现文件保存和加载（通过 CSquaresDoc::Serialize）。
    该文档将包含以下字符串：
        文件扩展名：      sqr
        文件类型 ID：        SdiSquares.Document
        主框架标题：  SdiSquares
        文档类型名称：       SdiSquares
        筛选器名称：         SdiSquares Files (*.sqr)
        文件的新短名称：SdiSquares
        文件类型长名称：SdiSquares.Document

SquaresView.h、SquaresView.cpp - 文档视图
    这些文件包含 CSquaresView 类。
    CSquaresView 对象用于查看 CSquaresDoc 对象。
